...
def trainingdata(ops, seed=101, ntrials=400, batchsize=20, useblocks=False, p_optout=0.0):
    ...
    delays = np.random.exponential(scale=ops['lam_pred'], size=(batchsize, ntrials))  # how long until reward arrives
    ...
    for k in range(batchsize):
        ...
        # maanav's case
        if ops['inputcase'] == 17:
            m5 = np.argwhere(vols_use[k, :] == 5)[:, 0]
            m10 = np.argwhere(vols_use[k, :] == 10)[:, 0]
            m20 = np.argwhere(vols_use[k, :] == 20)[:, 0]
            m40 = np.argwhere(vols_use[k, :] == 40)[:, 0]
            m80 = np.argwhere(vols_use[k, :] == 80)[:, 0]

            input_k[0, tstart[m5], 0] = 1
            input_k[0, tstart[m10], 1] = 1
            input_k[0, tstart[m20], 2] = 1
            input_k[0, tstart[m40], 3] = 1
            input_k[0, tstart[m80], 4] = 1
        else:
            input_k[0, tstart, 0] = np.log(vols_use[k, :])

        d_rews = ninp - 3  # gives 5 if maanavs case.
        input_k[0, tstart, d_rews] = 1  # input for trial start
        input_k[0, :, d_rews + 1] = ops['w']  # input for previous reward
        input_k[0, tstop, d_rews + 1] += rews_use[k, :]  # input for previous reward
        input_k[0, tstop, d_rews + 2] = actions_use[k, :]  # input f
...
def transition_5state(state, prevstate):
    """
    a transition function to decide which state will come next in the 5state version of inference task
    deterministic progression from 1 --> 2--> 3 --> 4 --> 5 --> 4 --> 3 --> 2 --> 1
    @param state: (int) current state
    @param prevstate: (int) previous state encountered
    @return:
    """
    ...
    if state == 4:
        ...
        newstate = 1
    # traverse up states
    elif state > prevstate:
        newstate = state + 1
    # traverse down states
    else:
        newstate = state - 1
...
def transition_2state(state, prevstate):
    """
    a transition function to decide which state will come next in the 5state version of inference task
    deterministic progression from 1 --> 2--> 1. simple case
    @param state: (int) current state
    @param prevstate: (int) previous state encountered
    @return:
    """
    ...
    if state == 0:
        ...
        print('error in provided state. 0 or 1')
        newstate = None

    return newstate


def transition_wt(state, prevstate):
    """
    a transition function to decide which state will come next in the actual wait time task
    deterministic progression from 1 --> 2--> 1. simple case
    @param state: (int) current state
    @param prevstate: (int) previous state encountered
    @return:
...
def trainingdata_general(ops, seed=101, ntrials=400, batchsize=20, useblocks=False, p_optout=0.0, stateops=None):
    """
    training set to  perform block inference, but with generic structure of inputs, # blocks
    :param ops: (dict) ops dictionary with bits about task
    :param seed: (int) random seed for trials
    :param ntrials: (int) number of trials per sample
    :param batchsize: (int) number of independent samples
    :param useblocks: (bool) use block structure or not
    :param p_optout: (float) probability of trials being an "opt-out" trial, where reward is witheld
    :param stateops: (dict) containing keys for -state transition function, -vdict for offer vals per state
    :return:
    """
    np.random.seed(seed)

    if stateops is None:
        vdict = {0: [5, 10, 20, 40, 80], 1: [20, 40, 80], 2: [5, 10, 20]}
        stateops = {'vdict': vdict, 'statetrans_fun': transition_wt}

    vdict = stateops['vdict']
    statetrans_fun = stateops['statetrans_fun']

    ntmax = int(ops['T'] / ops['dt']) + 1
    tmesh = np.linspace(0, ops['T'], ntmax)  # time bins

    # set the master lists about optouts, rewards, delays, etc. add some padded extra trials for last batch
    vols = [np.random.choice(vdict[k], size=(batchsize, ntrials)) for k in vdict.keys()]  # volume of water reward
    vols = np.array(vols)
        ...
        for j in range(1, ntrials):
            if useblocks:
                ...
                if ind < blocklen:
                    ...
                    ind = 0
                    newblock = statetrans_fun(block, prevblock)
    ...
    if ops['inputcase'] == 11:
        ...
        # TODO: the tasks are not yet designed to handle the kindergarten tasks in any order
        # TODO: this was designed to only train after inference and regular kindergarten
    else:
        print('input case supported now is only 11 or 15. you provided '+str(ops['inputcase']))
        ninp = None
...
def update(updater, outputs, targets,  lambda_kind, lambda_pred, inp, lossinds_kind=None, lossinds_pred=3):
    """
    ...
    :param inp: (np.ndarray) inputs to network. used to find when trial start is. not used here
    :param lossinds_kind: (list) which indices of output to use to MSE loss on  kindergarten
    :param lossinds_pred: (int) which index of target to use for cross-ent loss of state prediciton
    :return: loss_np value of loss for that minibatch
    """

    if lossinds_kind is None:
        lossinds_kind = [0, 1, 2]
...
def update_trialstart(updater, outputs, targets,  lambda_kind, lambda_pred, inp, lossinds_kind=None, lossinds_pred=3):
    """
...
# duplicate of kindergarten. TODO: delete? no. needs specific update code and options
def train(ops, net, updater, si, inp, targ, device=torch.device('cpu'), nepoch=100, nsteps=100,
    ...
    :param updatefun: (func) update function in wt_pred
    :return:
    """
    ...
    lossinds_pred = ops['lossinds_pred']
